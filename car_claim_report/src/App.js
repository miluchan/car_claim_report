...
export default function App() {
  ...
  const loadExistingClaim = async (row) => {
    ...
    otherVehicleSeqRef.current = 1;
    if (ovRows && ovRows.length > 0) {
      setOtherVehicles(
        ovRows.map((r) => ({
          seq: otherVehicleSeqRef.current++,
          visible: true,
      ...
      setOtherVehicles([emptyOtherVehicle()]);
    }
    setHiddenSeqStack([]);
  ...
  const lookupPolicy = async () => {
    ...
    try {
      ...
      const isIssued = data.payment_status === "已出單"; // 1. 一定要已出單才能受理
      const valid = inCompulsory && inArbitrary && isIssued;
      setPolicyValid(valid);
      if (inCompulsory && inArbitrary && !isIssued) {
      ...
      // 4. 查詢到的是新事故案件（沒有進行中的舊案件），先把上一筆殘留的畫面清空，再帶入新資料
      resetClaimFieldsKeepPlate();

      // 2. 一定產生全新立案編號＋通報時間（走到這裡代表沒有進行中的案件，不能沿用舊的claimNo）
      const ymd = getTodayYmd();
      const { data: todayClaims } = await supabaseClient
        .from("claims")
        .select("claim_no")
        .like("claim_no", `${ymd}-%`);
      const seq = (todayClaims || []).length + 1;
      const newClaimNo = `${ymd}-${String(seq).padStart(3, "0")}`;
      const nowIso = new Date().toISOString();
      setClaimNo(newClaimNo);
      ...
      setPolicyCoverageTypes(Array.isArray(data.coverage_items) ? data.coverage_items : []);
    } catch (e) {
      setPolicyValid(false);
      alert("⚠️ 查詢過程發生錯誤，請稍後再試。");
    }
    setPolicyLookupLoading(false);
  };
  ...
  // 3. 對方車及事故狀況區塊（一案多筆）
  // ================================================================
  const otherVehicleSeqRef = useRef(1);
  const emptyOtherVehicle = () => ({
    seq: otherVehicleSeqRef.current++,
    visible: true,
    otherPlateNo: "",
    otherDriverName: "",
  ...
  const [otherVehicles, setOtherVehicles] = useState([emptyOtherVehicle()]);
  ...
  const addOtherVehicle = () => {
    if (hiddenSeqStack.length > 0) {
      ...
      setOtherVehicles((prev) => [...prev, emptyOtherVehicle()]);
    }
  };
  ...
  return (
    <div className="container py-4" style={{ maxWidth: "980px" }}>
            ...
            <div className="bg-white rounded-3 p-4 shadow-lg" style={{ maxWidth: "480px", width: "100%" }}>
              ...
              <div className="row small">
                ...
                <div className="col-12 mt-2">
                  <span className="text-muted">投保代號及險種（保額）：</span>
                  {policyCoverageTypes.length === 0 ? (
                    <span className="fw-bold">未查得明細</span>
                  ) : (
                    <ul className="mb-0 ps-3">
                      {policyCoverageTypes.map((c, i) => {
                        // 3. 顯示保額（從險種名稱的括號裡取，例如「第三人體傷險(200萬/400萬)」→ 200萬/400萬），
                        // 有些險種（如車體、竊盜）當初投保沒有存保額，這種就不顯示金額，只顯示險種名稱
      ...
      {/* ==================== 2. 駕駛人區塊 ==================== */}
      <h6 className="fw-bold text-dark mb-2">🧑‍✈️ 駕駛人</h6>
      <div className="row g-3 bg-light p-3 rounded mb-4 border">
