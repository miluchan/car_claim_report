...
export default function App() {
  ...
  const [loadedUpdatedAt, setLoadedUpdatedAt] = useState(null); // 1. 樂觀鎖：記錄目前畫面資料是根據資料庫哪個版本讀出來的
  ...
  const loadExistingClaim = async (row) => {
    ...
    setLoadedUpdatedAt(row.updated_at || null); // 1. 記下當下讀到的版本，儲存時用來偵測有沒有被別人搶先改過
  };
  ...
  const createClaimStub = async (basePayload) => {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      lastError = error;
      // 2. 這台車已經有其他人搶先建立未結案案件了（撞到「同車號只能一筆進行中」的資料庫限制）——
      // 不要重試新編號（那只會又造出第二筆重複案件），直接中止，讓外層改成把已存在的那筆帶出來
      if (error.code === "23505" && error.message && error.message.includes("idx_claims_one_open_per_plate")) {
        const err = new Error("PLATE_ALREADY_OPEN");
        err.code = "PLATE_ALREADY_OPEN";
        throw err;
      }
  ...
  const lookupPolicy = async () => {
    ...
    try {
      ...
      // 🔁 先查這台車有沒有尚未結案（也還沒被註銷）的案件：有→帶出目前進度繼續處理；沒有→視為全新報案
      ...
      try {
        ...
        // 2. 就在我們檢查完「沒有進行中案件」之後、真正寫入之前的這一瞬間，
        // 被別人搶先建立了同一台車的案件——不要另外再建一筆，直接把那筆帶出來，並讓這位使用者知道發生了什麼事
        if (stubErr && stubErr.code === "PLATE_ALREADY_OPEN") {
          alert("⚠️ 這台車剛剛已被其他人員受理，已為您帶出該筆案件，請勿重複建立。");
          const { data: justCreated } = await supabaseClient
            .from("claims")
            .select("*")
            .eq("plate_no", plateNo.trim())
            .eq("case_closed", false)
          ...
          if (justCreated && justCreated.length > 0) {
            await loadExistingClaim(justCreated[0]);
          }
          setPolicyLookupLoading(false);
          return;
        }
      ...
      setLoadedUpdatedAt(stub.data.updated_at || null);
  ...
  // 4. 遠端簽署（簡訊/LINE）——同樣先過濾已簽署的文件，不用再發連結
  const isCurrentDocSigned = () => (signDocType === "理賠申請書" ? claimFormSignedStatus : settlementSignedStatus);
  const buildSignMessage = () => {
    const url = `${window.location.origin}${window.location.pathname}?signClaimId=${claimId}&signDocType=${encodeURIComponent(signDocType)}`;
    return `【理賠協助】請點選下方連結，完成「${signDocType}」線上簽署：\n${url}`;
  };
  const sendSignSms = () => {
    if (isCurrentDocSigned()) {
      alert(`ℹ️ ${signDocType} 已完成簽署，不需要重新簽署，也不用再發送簽署連結。`);
      return;
    }
    if (!claimId) {
      alert("⚠️ 請先按下方「儲存立案資料」建立案件，才能產生連結。");
    ...
    const phone = driverPhone || insuredPhone;
    if (!phone) {
      alert("⚠️ 查無可發送的電話號碼，請先填寫駕駛人或被保險人電話。");
      return;
    }
    window.location.href = `sms:${phone}?body=${encodeURIComponent(buildSignMessage())}`;
  };
  const sendSignLine = () => {
    if (isCurrentDocSigned()) {
      alert(`ℹ️ ${signDocType} 已完成簽署，不需要重新簽署，也不用再發送簽署連結。`);
    ...
    window.open(`https://line.me/R/msg/text/?${encodeURIComponent(buildSignMessage())}`, "_blank");
  };
  ...
  // 4. 遠端簽署客戶端：透過 ?signClaimId=&signDocType= 這組網址參數載入，跟AI引導拍照的客戶自助頁是同一種模式
  const [remoteSignData, setRemoteSignData] = useState(null);
  const [remoteSignLoaded, setRemoteSignLoaded] = useState(false);
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const scid = params.get("signClaimId");
    const sdt = params.get("signDocType");
    if (!scid || !sdt) return;
    (async () => {
      const { data: claimRow } = await supabaseClient.from("claims").select("*").eq("id", scid).maybeSingle();
      const { data: sigRows } = await supabaseClient
        .from("claim_signatures")
        .select("document_type")
        .eq("claim_id", scid);
      const already = (sigRows || []).some((r) => r.document_type === sdt);
      setRemoteSignData({ claimId: parseInt(scid, 10), docType: sdt, claim: claimRow, alreadySigned: already });
      setRemoteSignLoaded(true);
    })();
  }, []);

  // 5-2. 簽署畫面（不管是內部 or 遠端連結）開著的時候，直接鎖住整個頁面不能捲動。
  // 光靠畫布的 touch-action:none 跟 preventDefault() 在部分手機瀏覽器不夠可靠
  // （瀏覽器常把 touchmove 事件當成「passive」處理，這時 preventDefault() 會被忽略），
  // 這裡改成從根本上讓頁面在簽署當下完全不能滑動，簽完/關閉才恢復。
  const isSignUiActive = showSignModal || (remoteSignLoaded && remoteSignData?.claim && !remoteSignData.alreadySigned);
  useEffect(() => {
    if (isSignUiActive) {
  ...
  const submitRemoteSignature = async () => {
    if (!isSigned) {
      alert("⚠️ 請先完成簽名。");
      return;
    }
    ...
    try {
      ...
      await supabaseClient.from("claim_signatures").insert([
        {
          claim_id: remoteSignData.claimId,
          document_type: remoteSignData.docType,
          otp_verified: true,
      ...
      setRemoteSignData((prev) => ({ ...prev, alreadySigned: true }));
    } catch (e) {
      alert("⚠️ 簽署失敗，請稍後再試。");
    }
  };
  ...
  const sendOtpCode = () => {
    ...
    setOtpCode(code);
    setOtpPhase("verify");
    // 4. 遠端簽署模式下，畫面上沒有駕駛人/被保險人這些內部狀態，要改用連結帶進來那筆案件本身的電話
    const remotePhone = remoteSignData?.claim?.driver_phone || remoteSignData?.claim?.insured_phone;
    const phone = remoteSignData ? remotePhone : driverPhone || insuredPhone;
    alert("📱（模擬簡訊發送）驗證碼已發送至 " + (phone || "您填寫的手機號碼") + "\n\n測試用驗證碼：" + code);
  };
  const verifyOtpCode = () => {
    if (otpInput === otpCode) {
      setOtpVerified(true);
  ...
  // 5. 手機上簽名觸點位移的根本原因：canvas的「內部繪圖解析度」(width/height屬性=460x160)
  // 跟它實際顯示的CSS尺寸(w-100，手機上通常比460px窄很多)不一致，
  // 原本沒有把這個縮放比例算進去，導致手指點的位置跟畫出來的線條對不上。
  ...
  const startDrawing = (e) => {
    if (e.touches) e.preventDefault(); // 避免手機在畫布上滑動時，頁面跟著捲動/縮放造成畫面跳動
  ...
  const handleCloseCase = async () => {
    ...
    // 1. 一定要有決算金額才能結案
    if (finalSettlementAmount === "" || isNaN(parseFloat(finalSettlementAmount))) {
      alert("⚠️ 請先填寫決算金額，才能結案。");
      return;
    }
  ...
  // 5. 註銷立案：不刪除任何資料，只加註記；註銷後這個車號才會空出來，可以重新受理新案件
  const handleVoidClaim = async () => {
    if (!claimId) {
      alert("⚠️ 目前沒有正在處理的案件可以註銷。");
      return;
    }
    const confirmed = window.confirm(`確定要註銷案件 ${claimNo} 嗎？資料不會被刪除，只會標記為已註銷，畫面將會清空。`);
    if (!confirmed) return;
    try {
      const { error } = await supabaseClient
        .from("claims")
        ...
        .eq("id", claimId);
      if (error) {
        alert("⚠️ 註銷失敗：" + error.message);
        return;
      }
      alert("✅ 案件 " + claimNo + " 已註銷。");
      resetClaimForm();
    } catch (e) {
      alert("⚠️ 註銷過程發生未預期錯誤。");
    }
  ...
  // 2. 未決案查詢：一覽表，直接列出所有「尚未結案」的案件，不用先輸入關鍵字才查得到；
  // 也可以額外用車號/立案編號篩選縮小範圍。[詳細內容]只是唯讀檢視，不會把這筆案件帶進主畫面繼續處理。
  const [showClaimSearchModal, setShowClaimSearchModal] = useState(false);
  ...
  const [claimDetailView, setClaimDetailView] = useState(null); // 唯讀詳細內容

  const runClaimSearch = async () => {
    setClaimSearchLoading(true);
    try {
      const kw = claimSearchKeyword.trim();
      let query = supabaseClient.from("claims").select("*").eq("case_closed", false).eq("voided", false);
      if (kw) {
        query = query.or(`plate_no.eq.${kw},claim_no.ilike.%${kw}%`);
      }
      const { data, error } = await query.order("id", { ascending: false }).limit(50);
      if (error) {
        alert("⚠️ 查詢失敗：" + error.message);
      } else {
        setClaimSearchResults(data || []);
      }
    } catch (e) {
      alert("⚠️ 查詢過程發生未預期錯誤。");
    }
  ...
  const openClaimSearch = () => {
    setClaimSearchKeyword("");
    setClaimSearchResults([]);
    setClaimDetailView(null);
    setShowClaimSearchModal(true);
    runClaimSearch(); // 一開啟就直接列出目前所有未決案，不用先打字才看得到
  };

  // 只顯示唯讀內容，不呼叫 loadExistingClaim，不會設定 claimId，也就不能繼續編輯/儲存
  const openClaimDetailFromSearch = (row) => {
    setClaimDetailView(row);
  };
  ...
  const saveClaim = async () => {
    ...
    try {
      ...
      let data, error;
      if (claimId) {
        // 1. 樂觀鎖檢查：儲存前先看資料庫裡「目前的版本」跟我畫面上讀到的版本是不是同一個。
        // 如果不一樣，代表在我看畫面的這段期間，已經有別人存過這筆案件了——直接擋下來，
        // 不要盲目覆蓋掉別人的修改，這是目前沒有登入/身分系統下，能做到最務實的保護機制。
        const { data: currentRow, error: checkError } = await supabaseClient
          .from("claims")
          .select("updated_at")
          .eq("id", claimId)
          .single();
        if (checkError) {
          alert("⚠️ 儲存前檢查失敗：" + checkError.message);
          setSaving(false);
          return;
        }
        if (loadedUpdatedAt && currentRow.updated_at && currentRow.updated_at !== loadedUpdatedAt) {
          alert(
            "⚠️ 這筆案件在您處理的期間，已經被其他人員儲存過最新的變更。\n為避免覆蓋掉對方的修改，這次儲存已取消，請重新查詢這筆案件取得最新內容後再繼續。"
          );
          setSaving(false);
          return;
        }
        ...
        ({ data, error } = await supabaseClient
          .from("claims")
          .update({ ...payload, updated_at: nowIso2 })
          .eq("id", claimId)
          .select()
          .single());
      } else {
      ...
      if (data.id) {
        ...
        // 6. 全部欄位都空白／未勾選的對方車，不用存進資料庫（通常是使用者新增了空白列但沒有真的填資料）
        const isVehicleEmpty = (v) =>
          !v.otherPlateNo.trim() &&
          !v.otherDriverName.trim() &&
          !v.otherDriverGender.trim() &&
          !v.otherDriverPhone.trim() &&
          !v.injuryFlag &&
          !v.hospitalizedFlag &&
          !v.towedFlag &&
          !v.ownVehicleRepairFlag &&
          !v.ownVehicleDrivableFlag;
        const nonEmptyVehicles = otherVehicles.filter((v) => !isVehicleEmpty(v));
        if (nonEmptyVehicles.length > 0) {
          await supabaseClient.from("claim_other_vehicles").insert(
            nonEmptyVehicles.map((v) => ({
              claim_id: data.id,
  ...
  const signClaimIdParam = urlParams.get("signClaimId");
  if (signClaimIdParam) {
    if (!remoteSignLoaded) {
      return (
        <div className="container py-5 text-center">
          <div className="spinner-border text-primary" role="status"></div>
          <div className="mt-2 text-muted">載入中...</div>
        </div>
      );
    }
    if (!remoteSignData?.claim) {
      return (
        <div className="container py-5 text-center">
          <div className="fs-1 mb-2">⚠️</div>
          <div className="fw-bold">找不到這筆案件，連結可能已失效。</div>
        </div>
      );
    }
    if (remoteSignData.alreadySigned) {
      return (
        <div className="container py-5 text-center" style={{ maxWidth: "420px" }}>
          <div className="fs-1 mb-2">✅</div>
          <div className="fw-bold fs-5 mb-2">{remoteSignData.docType} 已完成簽署</div>
          <div className="small text-muted">不需要重複簽署，感謝您的配合，您可以關閉此頁面。</div>
        </div>
      );
    }
    return (
      <div className="container py-4" style={{ maxWidth: "480px" }}>
        <h5 className="fw-bold text-center text-primary mb-3">✍️ {remoteSignData.docType} 線上簽署</h5>
        <div className="border rounded p-3 mb-3 bg-light small">
          <div>立案編號：{remoteSignData.claim.claim_no}</div>
          <div>車號：{remoteSignData.claim.plate_no}</div>
          <div>被保險人：{remoteSignData.claim.insured_name}</div>
        </div>
        ...
        <div className="small text-muted mb-1">請在藍色虛線框內用手指手寫簽名：</div>
        <canvas
          ref={canvasRef}
          width={460}
          height={160}
        ...
        <div className="row g-2">
          ...
          <div className="col-6">
            <button type="button" className="btn btn-primary w-100 fw-bold" onClick={submitRemoteSignature}>
              確認送出
            </button>
          </div>
        </div>
          ...
          <div className="modal d-block show bg-black bg-opacity-75" style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 100080, overflowY: "auto" }}>
            <div className="d-flex align-items-center justify-content-center min-vh-100 p-3">
              <div className="bg-white rounded-3 p-4 shadow-lg" style={{ maxWidth: "400px", width: "100%" }}>
                ...
                {otpPhase === "send" && (
                  <>
                    <p className="small text-muted">將發送驗證碼至您登記的手機號碼</p>
                    <button type="button" className="btn btn-primary w-100 fw-bold" onClick={sendOtpCode}>
                      發送驗證碼
                    </button>
                  </>
                )}
    ...
    <div className="container py-4" style={{ maxWidth: "980px" }}>
      ...
      <div className="row g-2 align-items-end bg-light border rounded p-3 mb-4">
        ...
        <div className="col-md-2">
          <button type="button" className="btn btn-primary w-100 fw-bold" onClick={lookupPolicy} disabled={policyLookupLoading}>
            {policyLookupLoading ? "查詢中..." : "受理與查詢報案"}
          </button>
        </div>
        ...
        <div className="col-6 col-md-auto">
          <button type="button" className="btn btn-outline-success fw-bold w-100" onClick={sendGuideSms}>
            📱 簡訊通知
          </button>
        </div>
        <div className="col-6 col-md-auto">
          <button type="button" className="btn btn-outline-success fw-bold w-100" onClick={sendGuideLine}>
            💬 LINE通知
          </button>
        </div>
        <div className="col-12 d-flex justify-content-end gap-2 mt-2">
          <button type="button" className="btn btn-outline-secondary fw-bold" onClick={openClaimSearch}>
            🔍 未決案查詢
          </button>
          <button type="button" className="btn btn-outline-danger fw-bold" onClick={handleVoidClaim} disabled={!claimId}>
            🚫 註銷立案
          </button>
        </div>
      </div>
      ...
      {showClaimSearchModal && (
        <div className="modal d-block show bg-black bg-opacity-75" style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 100060, overflowY: "auto" }}>
          <div className="d-flex align-items-center justify-content-center min-vh-100 p-3">
            <div className="bg-white rounded-3 p-4 shadow-lg" style={{ maxWidth: "640px", width: "100%" }}>
              <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
                <h6 className="fw-bold text-primary mb-0">🔍 未決案查詢</h6>
                <button type="button" className="btn-close" onClick={() => setShowClaimSearchModal(false)} />
              </div>
              <div className="input-group mb-3">
                <input
                  type="text"
                  className="form-control"
                  placeholder="可輸入車號或立案編號篩選（留空＝顯示全部未決案）"
                  value={claimSearchKeyword}
                  onChange={(e) => setClaimSearchKeyword(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && !e.nativeEvent.isComposing) runClaimSearch();
                  }}
                />
                <button type="button" className="btn btn-primary fw-bold" onClick={runClaimSearch} disabled={claimSearchLoading}>
                  {claimSearchLoading ? "查詢中..." : "查詢"}
                </button>
              </div>

              {claimSearchResults.length === 0 ? (
                <div className="text-muted small">目前沒有未決案（尚未結案）的案件。</div>
              ) : (
                <div style={{ maxHeight: "420px", overflowY: "auto" }}>
                  {claimSearchResults.map((row) => {
                    const stages = [
                      { label: "已立案", done: true },
                      { label: "文件完備", done: Array.isArray(row.required_documents) && row.required_documents.length > 0 && row.required_documents.every((d) => d.checked) },
                      { label: "已預估", done: row.estimated_claim_amount != null },
                      { label: "已結案", done: !!row.case_closed },
                      { label: "已付款", done: !!row.payment_completed },
                    ];
                    return (
                      <div key={row.id} className="border rounded p-3 mb-2">
                        <div className="row small g-2">
                          <div className="col-6 col-md-3"><span className="text-muted">立案編號：</span><span className="fw-bold">{row.claim_no}</span></div>
                          <div className="col-6 col-md-3"><span className="text-muted">通報時間：</span><span className="fw-bold">{row.report_time ? new Date(row.report_time).toLocaleString("zh-TW") : "－"}</span></div>
                          <div className="col-6 col-md-3"><span className="text-muted">車號：</span><span className="fw-bold">{row.plate_no}</span></div>
                          <div className="col-6 col-md-3"><span className="text-muted">保單號碼：</span><span className="fw-bold">{row.quotation_no || "－"}</span></div>
                          <div className="col-6 col-md-3"><span className="text-muted">被保人：</span><span className="fw-bold">{row.insured_name || "－"}</span></div>
                          <div className="col-6 col-md-3"><span className="text-muted">駕駛人：</span><span className="fw-bold">{row.driver_name || "－"}</span></div>
                        </div>
                        <div className="d-flex flex-wrap align-items-center gap-2 mt-2">
                          {stages.map((s, i) => (
                            <div key={i} className="d-flex align-items-center">
                              <span className={s.done ? "fw-bold text-dark small" : "text-muted small"} style={s.done ? {} : { opacity: 0.45 }}>
                                {s.done ? "🟢" : "⚪"} {s.label}
                              </span>
                              {i < stages.length - 1 && <span className="mx-1 text-muted small">→</span>}
                            </div>
                          ))}
                        </div>
                        <button
                          type="button"
                          className="btn btn-sm btn-outline-primary fw-bold mt-2"
                          onClick={() => openClaimDetailFromSearch(row)}
                        >
                          詳細內容
                        </button>
                      </div>
              ...
              {/* 唯讀詳細內容：只是檢視，不會把這筆案件帶進主畫面繼續處理 */}
              {claimDetailView && (
                <div className="border-top pt-3 mt-3">
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    <h6 className="fw-bold text-primary mb-0">📄 詳細內容（唯讀）</h6>
                    <button type="button" className="btn-close" onClick={() => setClaimDetailView(null)} />
                  </div>
                  <div className="row small g-2 bg-light rounded p-3">
                    <div className="col-6"><span className="text-muted">立案編號：</span><span className="fw-bold">{claimDetailView.claim_no}</span></div>
                    <div className="col-6"><span className="text-muted">通報時間：</span><span className="fw-bold">{claimDetailView.report_time ? new Date(claimDetailView.report_time).toLocaleString("zh-TW") : "－"}</span></div>
                    <div className="col-6"><span className="text-muted">車號：</span><span className="fw-bold">{claimDetailView.plate_no}</span></div>
                    <div className="col-6"><span className="text-muted">保單號碼：</span><span className="fw-bold">{claimDetailView.quotation_no || "－"}</span></div>
                    <div className="col-6"><span className="text-muted">被保人：</span><span className="fw-bold">{claimDetailView.insured_name || "－"}（{claimDetailView.insured_gender || "－"}）</span></div>
                    <div className="col-6"><span className="text-muted">被保人電話：</span><span className="fw-bold">{claimDetailView.insured_phone || "－"}</span></div>
                    <div className="col-6"><span className="text-muted">駕駛人：</span><span className="fw-bold">{claimDetailView.driver_name || "－"}（{claimDetailView.driver_gender || "－"}）</span></div>
                    <div className="col-6"><span className="text-muted">駕駛人電話：</span><span className="fw-bold">{claimDetailView.driver_phone || "－"}</span></div>
                    <div className="col-6"><span className="text-muted">車種／廠牌車系：</span><span className="fw-bold">{claimDetailView.vehicle_type || "－"}／{claimDetailView.brand_series || "－"}</span></div>
                    <div className="col-6"><span className="text-muted">事故時間／地點：</span><span className="fw-bold">{claimDetailView.accident_time || "－"}／{claimDetailView.accident_location || "－"}</span></div>
                    <div className="col-6"><span className="text-muted">報案人電話：</span><span className="fw-bold">{claimDetailView.reporter_phone || "－"}</span></div>
                    <div className="col-6"><span className="text-muted">複雜度分流：</span><span className="fw-bold">{claimDetailView.complexity_tier || "尚未分流"}</span></div>
                    <div className="col-6"><span className="text-muted">預估金額：</span><span className="fw-bold">{claimDetailView.estimated_claim_amount != null ? "NT$ " + Number(claimDetailView.estimated_claim_amount).toLocaleString() : "－"}</span></div>
                  </div>
                  <div className="small text-muted mt-2">ℹ️ 這裡僅供檢視，若要繼續處理此案件，請到主畫面用車號重新受理查詢。</div>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="row g-2 align-items-end bg-light border rounded p-3 mb-4">
        ...
        <div className="col-6 col-md-2">
          <button type="button" className="btn btn-outline-success w-100 fw-bold" onClick={sendSignSms}>
            📱 簡訊
          </button>
        </div>
        <div className="col-6 col-md-2">
          <button type="button" className="btn btn-outline-success w-100 fw-bold" onClick={sendSignLine}>
            💬 LINE
          </button>
        </div>
        <div className="col-12 small text-muted">遠端簽署：發送連結讓客戶自行完成線上簽署</div>
