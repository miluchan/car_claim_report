...
export default function App() {
  ...
  // 5／6. 立案查詢：依車號或立案編號搜尋，列出關鍵資訊＋處理進度，[詳細內容]直接把該筆帶回主畫面
  ...
  const runClaimSearch = async () => {
    ...
    try {
      ...
      const { data, error } = await supabaseClient
        .from("claims")
        .select("*")
        .or(`plate_no.eq.${kw},claim_no.ilike.%${kw}%`)
        .order("id", { ascending: false })
        .limit(20);
      if (error) {
  ...
  const openClaimDetailFromSearch = async (row) => {
    await loadExistingClaim(row);
    setShowClaimSearchModal(false);
  };
        ...
        <div className="modal d-block show bg-black bg-opacity-75" style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 100060, overflowY: "auto" }}>
          <div className="d-flex align-items-center justify-content-center min-vh-100 p-3">
            <div className="bg-white rounded-3 p-4 shadow-lg" style={{ maxWidth: "640px", width: "100%" }}>
              <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
                <h6 className="fw-bold text-primary mb-0">🔍 立案查詢</h6>
              ...
              {claimSearchResults.length === 0 ? (
                <div className="text-muted small">請輸入車號或立案編號後查詢。</div>
              ) : (
                    ...
                    return (
                      <div key={row.id} className="border rounded p-3 mb-2">
                        <div className="row small g-2">
                          ...
                          {row.voided && <div className="col-12"><span className="badge bg-secondary">已註銷</span></div>}
                        </div>
